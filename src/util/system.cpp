...
const char * const BEANS_CONF_FILENAME = "beans.conf";
...
{
    // Windows: C:\Users\Username\AppData\Roaming\Beans
    // macOS: ~/Library/Application Support/Beans
    // Unix-like: ~/.beans
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Beans";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/Beans";
#else
    // Unix-like
    return pathRet / ".beans";
#endif
#endif
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure Beans Core copyright is not removed by accident
    if (copyright_devs.find("Beans Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Beans Core developers";
    }
