...
static int AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        if (!gArgs.IsArgSet("-version")) {
            strUsage += "\n"
                "Usage:  beans-cli [options] <command> [params]  Send command to " PACKAGE_NAME "\n"
                "or:     beans-cli [options] -named <command> [name=value]...  Send command to " PACKAGE_NAME " (with named arguments)\n"
                "or:     beans-cli [options] help                List commands\n"
                "or:     beans-cli [options] help <command>      Get help for a command\n";
...
public:
    ...
    UniValue ProcessReply(const UniValue& reply) override
    {
        ...
        if (!nodes.empty() && nodes.at(0)["network"].isNull()) {
            throw std::runtime_error("-addrinfo requires beansd server to be running v22.0 and up");
        }
...
public:
    ...
    const std::string m_help_doc{
        ...
        "> beans-cli -netinfo 1\n\n"
        "Full dashboard\n"
        + strprintf("> beans-cli -netinfo %d\n\n", MAX_DETAIL_LEVEL) +
        "Full live dashboard, adjust --interval or --no-title as needed (Linux)\n"
        + strprintf("> watch --interval 1 --no-title beans-cli -netinfo %d\n\n", MAX_DETAIL_LEVEL) +
        "See this help\n"
        "> beans-cli -netinfo help\n"};
};
