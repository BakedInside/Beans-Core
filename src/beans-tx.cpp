...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        std::string strUsage = PACKAGE_NAME " beans-tx utility version " + FormatFullVersion() + "\n";
        if (!gArgs.IsArgSet("-version")) {
            strUsage += "\n"
                "Usage:  beans-tx [options] <hex-tx> [commands]  Update hex-encoded beans transaction\n"
                "or:     beans-tx [options] -create [commands]   Create hex-encoded beans transaction\n"
                "\n";
