...
This guide describes how to build beansd, command-line utilities, and GUI on macOS
...
These tools must be installed in order to build Beans Core from source.
...
### 4. Clone Beans repository

`git` should already be installed by default on your system.
Now that all the required dependencies are installed, let's clone the Beans Core repository to a directory.
...
It is not necessary to build wallet functionality to run `beansd` or  `beans-qt`.
To enable legacy wallets, you must install `berkeley-db@4`.
To enable [descriptor wallets](https://github.com/beans/beans/blob/master/doc/descriptors.md), `sqlite` is required.
...
Beans Core includes a GUI built with the cross-platform Qt Framework.
...
See the notes in [#7714](https://github.com/beans/beans/issues/7714).
...
You can deploy a `.dmg` containing the Beans Core application using `make deploy`.
...
There are many ways to configure Beans Core, here are a few common examples:
...
Run the following in your terminal to compile Beans Core:
...
Beans Core should now be available at `./src/beansd`.
If you compiled support for the GUI, it should be available at `./src/qt/beans-qt`.

The first time you run `beansd` or `beans-qt`, it will start downloading the blockchain.
This process could take many hours, or even days on slower than average systems.

By default, blockchain and wallet data files will be stored in:

``` bash
/Users/${USER}/Library/Application Support/Beans/
```
...
./src/qt/beans-qt -server # Starts the beans-qt server mode, allows beans-cli control
```
