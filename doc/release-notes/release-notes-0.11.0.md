Beans Core version 0.11.0 is now available from:

  <https://beans.org/bin/beans-core-0.11.0/>
...
defaults to 0.00001. This will cause transactions with fewer BNS/kB fee to be
...
For example, add the following to `beans.conf`:
...
related to the blockchain in the beans system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Beans Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Beans Core will 
...
For an example script see [beans-submittx](https://github.com/laanwj/beans-submittx).
...
- #5334 `cf87536` libbeansconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new beans-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only BeansAmountField
- #5753 `0f44672` Add beans logo to about screen
...
- #5809 `46bfbe7` Add beans-cli man page
...
- #5849 `41259ca` contrib/init/beansd.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Beans-Qt.app to Beans-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for beans-tx)
