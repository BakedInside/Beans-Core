...
The Beans Core daemon can be configured to act as a trusted "border
router", implementing the beans wire protocol and relay, making
...
The ZeroMQ feature in Beans Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building beansd:
...
For instance:
    ...
    $ beansd -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/beansd.tx.raw \
...
These options can also be provided in beans.conf.
...
From the perspective of beansd, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into beansd directly. Furthermore, no information is
...
using. Beansd appends an up-counting sequence number to each
