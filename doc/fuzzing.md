...
# Fuzzing Beans Core using afl++

## Quickstart guide

To quickly get started fuzzing Beans Core using [afl++](https://github.com/AFLplusplus/AFLplusplus):
...
## Fuzzing the Beans Core P2P layer using Honggfuzz NetDriver

Honggfuzz NetDriver allows for very easy fuzzing of TCP servers such as Beans
Core without having to write any custom fuzzing harness. The `beansd` server
...
```sh
$ mkdir beans-honggfuzz-p2p/
$ cd beans-honggfuzz-p2p/
$ git clone https://github.com/beans/beans
$ cd beans/
$ ./autogen.sh
$ git clone https://github.com/google/honggfuzz
...
$ git apply << "EOF"
diff --git a/src/beansd.cpp b/src/beansd.cpp
...
$ honggfuzz/honggfuzz --exit_upon_crash --quiet --timeout 4 -n 1 -Q \
      -E HFND_TCP_PORT=18444 -f inputs/ -- \
          src/beansd -regtest -discover=0 -dns=0 -dnsseed=0 -listenonion=0 \
...
Beans Core participates in Google's [OSS-Fuzz](https://github.com/google/oss-fuzz/tree/master/projects/beans-core)
...
because Beans is a live P2P network, and not just standalone local software,
